...
class BackProjection(torch.autograd.Function):
    ...
    def forward(ctx, input):
        device = input.device
        input = input.reshape(beijingAngleNum, beijingSubDetectorSize[1], beijingPlanes, beijingSubDetectorSize[0]+beijingGap).permute(0,2,1,3).reshape(1, 1, beijingAngleNum*beijingPlanes, beijingSubDetectorSize[1], beijingSubDetectorSize[0]+beijingGap)
        volume = projector.backward(input, volumeSize.to(device), detectorSize.to(device), parameters.to(device), beijingGap, device.index)
        return volume
...
class BeijingGeometryWithFBP(torch.nn.Module):
    def __init__(self):
        ...
        self.ramp = torch.nn.Parameter(self.__conv__((beijingSubDetectorSize[0]+beijingGap)*beijingPlanes), requires_grad=False)

    def forward(self, x, p):
        residual = p
        residual = residual.view(1, 1, beijingAngleNum, beijingSubDetectorSize[1], (beijingSubDetectorSize[0]+beijingGap)*beijingPlanes)
        residual = residual * self.cosWeight
        residual = residual.view(1, 1, beijingAngleNum * beijingSubDetectorSize[1], (beijingSubDetectorSize[0]+beijingGap)*beijingPlanes)
        residual = torch.nn.functional.conv2d(residual, self.ramp, stride=1, padding=(0, int((beijingSubDetectorSize[0]+beijingGap)*beijingPlanes/2)))
        residual = residual.view(1, 1, beijingAngleNum*beijingPlanes, beijingSubDetectorSize[1], beijingSubDetectorSize[0]+beijingGap)
        return self.lamb * BackProjection.apply(residual)

    def __cosWeight__(self):
        cosine = np.zeros([1,1,beijingAngleNum,beijingSubDetectorSize[1],(beijingSubDetectorSize[0]+beijingGap)*beijingPlanes], dtype=np.float32)
