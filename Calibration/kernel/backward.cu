...
__global__ void backwardKernel(float* volume, float* sino, float* projectVector, const uint angleNum, const uint2 volumeSize, const float2 volumeCenter, const uint detectorSize, const float detectorCenter){
    ...
    #if defined DEBUG
    if(angleNum==0&&volumeIdx.x==0&&volumeIdx.y==0)
      printf("pos = %f\n", angleNum * detectorSize + u/v - detectorCenter);
    #endif
    ...
    atomicAdd(volume+volumeIdx.y*volumeSize.x+volumeIdx.x, gsino);
}
...
torch::Tensor backward(torch::Tensor sino, torch::Tensor projectVector){
    ...
    auto out = torch::zeros({volumeSize.x,volumeSize.y}).to(sino.device());
    ...
    for(int idx=0;idx<angleNum;idx++){
      ...
      backwardKernel<<<gridSize, blockSize>>>(outPtr, sinoPtr, projectVectorPtr, idx, volumeSize, volumeCenter, detectorSize, detectorCenter);
    }
